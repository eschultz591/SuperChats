...
vector<user> users;
      ...
      socket_(io_context)
  {
    ...
    void attempt_delete(char msgd[50])
    {
        ...
        {
       //     std::cout <<x.s<<"="<<cr_name<<"]";
            if(strcmp(x.s,cr_name) == 0 &&x.isLobby==false && x.user_count == 0)
            {
         //       std::cout << "will" << x.s << "delete I promise\n";
                ChatRooms.erase(ChatRooms.begin() + cr);
...
int x = 0;
 ...
 strcpy(U.s, CN);
    ...
    void create_lobby(char msgd[50])
    {
       // std::cout << msgd;
  ...
  void do_read_body( )
  {
    async_read(socket_,
        ...
        [this](std::error_code ec, std::size_t /*length*/)
        {
          ...
          if (!ec)
          {
          //  mvwprintw(currWindow, i++, 1, read_msg_.body());
		//			wmove(currWindow, i++, 1);
		//		wrefresh(currWindow);

			//		mvwprintw(currWindow, i, userMessageName.size()+1, text); //print what is being written in the messages window
            ...
            if (read_msg_.data()[5] == 'B')
              ...
              {
              create_lobby(read_msg_.data());
              }
            if (read_msg_.data()[5] == 'S')
            {

            char cr[24];
                       // mvwprintw(currWindow, i++, 1, read_msg_.data());
            ...
            //mvwprintw(currWindow, i++, 1, headT);
            //mvwprintw(currWindow, i++, 1, read_msg_.data());
            //mvwprintw(currWindow, i++, 1, ChatRooms[(int)currCR].s);
            //mvwprintw(currWindow, i++, 1, ChatRooms[(int)currCR].modN);
            mvwprintw(currWindow, i++, strlen(name), read_msg_.body());
            wmove(currWindow, i++, 1);
            wrefresh(currWindow);
            strcpy(read_msg_.body(), " ");
